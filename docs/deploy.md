...
- **로그인 실패 카운터가 프로세스 메모리에 있다.** 인스턴스를 늘리면 각자 세므로 실효 임계값이 인스턴스 수만큼 늘어나고, 재배포하면 초기화된다. 지금은 단일 인스턴스라 성립한다. 늘릴 때는 Redis 같은 공용 저장소로 옮겨야 한다.
- **`admin` 계정이 알려진 이름으로 운영 DB에 있다.** 비밀번호는 `APP_INIT_DATA_PASSWORD`라 뚫리지 않지만, 아이디가 알려져 있으므로 표적이 된다. 실제로 쓰기 시작하면 샘플 데이터를 지우는 편이 낫다.
- **회원을 지우는 API가 없다.** 잘못 만든 계정은 DB에서 직접 지워야 한다.
